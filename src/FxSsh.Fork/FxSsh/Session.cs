...
namespace FxSsh
{
    public class Session
    {
        ...
        private bool _clientSupportsExtInfo = false;
        ...
        private void HandleMessage(KeyExchangeInitMessage message)
        {
            ...
            // RFC 8308: Check if client supports ext-info extension
            _clientSupportsExtInfo = message.KeyExchangeAlgorithms.Contains("ext-info-c");
        ...
        /// <summary>
        /// Sends SSH_MSG_EXT_INFO with server-sig-algs extension (RFC 8308)
        /// This allows modern SSH clients to know which signature algorithms are supported
        /// for user authentication, enabling RSA-SHA2 and other algorithms to work.
        /// </summary>
        private void SendExtInfoIfSupported()
        {
            if (!_clientSupportsExtInfo)
                return;

            // Get all public key algorithms we support for user authentication
            var supportedAlgorithms = _publicKeyAlgorithms.Keys.ToArray();

            var extInfo = ExtInfoMessage.CreateWithServerSigAlgs(supportedAlgorithms);
            SendMessage(extInfo);
        }
